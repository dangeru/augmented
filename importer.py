...
for f in glob.glob("post/*.txt"):
    ...
    out.write("INSERT INTO posts (title, content, author, date_posted, is_op, ip, tag) VALUES (" + con.escape(title) + ", "+con.escape(contents)+", "+con.escape(author)+", "+(make_datetime(rs[0][1]) if len(rs) > 0 else "CURRENT_TIMESTAMP()")+", TRUE, '', 'imported');\n")
